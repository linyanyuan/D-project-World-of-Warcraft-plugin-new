...
-- 12.1: SetDesaturation 全局函数已移除，改用 Texture:SetDesaturated 方法 + SetVertexColor 降级
-- 原 AceGUI CheckBox widget 的 6 处 SetDesaturation(texture, bool) 调用走此本地兼容封装
local SetDesaturation
do
    local SetDesaturated = function(texture, desaturation)
        local supported = texture:SetDesaturated(desaturation)
    ...
    SetDesaturation = SetDesaturated
end
